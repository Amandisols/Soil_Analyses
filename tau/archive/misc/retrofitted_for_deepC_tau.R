...
csvlist <- list()
r  csvdata <- read.csv(paste0('source/',nocsvs[a]), header = TRUE, sep = ",")
  csvlist[[a]] <- csvdata
}
...
innerjoin <- read.csv('values_elem/hbelem210305_deepC.csv')
...
avgs <- cbind(Pedon = avg$pedon, TDepth = avg$top.cm, BDepth = avg$base.cm, Depth = avg$base.cm+((avg$top.cm-avg$base.cm)/2), Name = avg$newname, Ti = avg$Ti, Zr = avg$Zr, Y = avg$Y, Ratio = ratioavg, subsavg, stringsAsFactors = FALSE)
  ...
  write.csv(dftaus, file = paste0("values_tau/hb/taus_alldeep.csv"))
